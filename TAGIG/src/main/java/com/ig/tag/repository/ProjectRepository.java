...
public interface ProjectRepository extends JpaRepository<ProjectsEntity, Long> {
	...
	@Query(value = "select * FROM projects p INNER JOIN teams t ON p.id = ?1 AND t.id=?2", nativeQuery=true)
	public ProjectsEntity findAllByTeamAndProject(int project, int team);
